...
class ProductAnalyticsViewModel {
    ...
    var productSalesCount: [String: Int] {
        var salesDict: [String: Int] = [:]
        for order in orders {
            for item in order.items { salesDict[item.product.name, default: 0] += item.quantity }
        }
        return salesDict
    }

    var bestSellers: [(productName: String, quantitySold: Int)] {
        return productSalesCount.map { (productName: $0.key, quantitySold: $0.value) }.sorted { $0.quantitySold > $1.quantitySold }
    }
