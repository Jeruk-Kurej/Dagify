...
class MockStoreRepository: StoreProtocol {
    ...
    func fetchStore(storeId: String) async throws -> Store {
        if shouldThrowError {
            throw NSError(domain: "MockStoreError", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to fetch store"])
        }
        return dummyStore
    }
    ...
    func addBranch(storeId: String, branch: Branch) async throws -> Bool {
        if shouldThrowError {
            throw NSError(domain: "MockStoreError", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to add branch"])
        }
