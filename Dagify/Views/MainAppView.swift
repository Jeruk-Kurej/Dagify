...
struct MainAppView: View {
    ...
    var body: some View {
                ...
                TabView(selection: $selectedTab) {
                    ...
                    DashboardView(
                        viewModel: DashboardViewModel(
                            ...
                            storeProtocol: operationalService
                        ),
                        storeId: storeId,
                    ...
                    .tag(AppMenu.dashboard)

                    CashflowView(
                        viewModel: CashflowViewModel(
                            cashProtocol: cashflowService
                        ),
                        branchId: activeBranchId
                    )
                    .id(activeBranchId)
                    ...
                    .tag(AppMenu.operational)

                    // ✅ KODE BERUBAH: Mengirimkan Protocol langsung ke SettingsView
                    SettingsView(
        ...
        .task {
            await setupInitialBranch()
        }
    }

    private func setupInitialBranch() async {
        do {
            let storeInfo = try await operationalService.fetchStore(
                storeId: storeId
            )
            if let firstBranch = storeInfo.branches.first {
                activeBranchId = firstBranch.id
            }
        } catch {
            print("Gagal memuat cabang utama.")
        }
