        ...
        List {
            Section {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Nama Produk").font(.caption).foregroundColor(
                        Color(hex: "#6B7280")
                    )
                    TextField("Cth: Kopi Susu Aren", text: $productName).font(
                        .body
                    ).foregroundColor(Color(hex: "#111827"))
                }.padding(.vertical, 4)

                VStack(alignment: .leading, spacing: 8) {
                    Text("Harga Jual (Rp)").font(.caption).foregroundColor(
                        Color(hex: "#6B7280")
                    )
                    TextField("Cth: 18000", text: $productPrice).keyboardType(
                        .numberPad
            ...
            Section {
                HStack {
                    Image(systemName: "link").foregroundColor(
                        Color(hex: "#00A3A3")
                    )
                    Text(
                        "Fitur integrasi resep & bahan baku otomatis akan segera hadir."
                    ).font(.footnote).foregroundColor(Color(hex: "#6B7280"))
                }.padding(.vertical, 8)
            } header: {
            ...
            Section {
                ...
                .listRowBackground(
                    (productName.isEmpty || productPrice.isEmpty)
                        ? Color(hex: "#E5E7EB") : Color(hex: "#00A3A3")
                )
            ...
            if let err = viewModel.errorMessage {
                Section {
                    Text(err).font(.footnote).foregroundColor(
                        Color(hex: "#EF4444")
                    )
