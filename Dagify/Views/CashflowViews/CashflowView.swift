...
struct CashflowView: View {
    ...
    // ✅ STATE BARU: Untuk menyimpan data transaksi yang di-klik
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if viewModel.chartData.isEmpty {
                            ...
                            HStack(spacing: 16) {
                                HStack(spacing: 4) {
                                    Circle().fill(Color(hex: "#10B981")).frame(
                                        width: 8,
                                        height: 8
                                    )
                                    Text("Pemasukan").font(.caption2)
                                        .foregroundColor(.gray)
                                }
                                HStack(spacing: 4) {
                                    Circle().fill(Color(hex: "#EF4444")).frame(
                                        width: 8,
                                        height: 8
                                    )
                                    Text("Pengeluaran").font(.caption2)
                                        .foregroundColor(.gray)
                                }
                                HStack(spacing: 4) {
                                    Circle().fill(Color.blue).frame(
                                        width: 8,
                                        height: 8
                                    )
                                    Text("Cash Flow").font(.caption2)
                                        .foregroundColor(.gray)
                                }
                            }
                            .frame(maxWidth: .infinity, alignment: .center)
                    ...
                    HStack {
                        Button(action: {
                            ...
                            Image(systemName: "chevron.left").font(
                                ...
                                Color(hex: "#00A3A3").opacity(0.1)
                            ).foregroundColor(Color(hex: "#00A3A3")).clipShape(
                                Circle()
                            )
                        }
                        Spacer()
                        Text(viewModel.currentMonthString).font(.title3)
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        HStack {
                            Text("Riwayat Transaksi").font(.headline)
                                .foregroundColor(Color(hex: "#111827"))
                            Spacer()
                            Button("Tambah Manual") { isShowingAdd = true }
                        ...
                        if viewModel.isLoading
                            && viewModel.filteredRecords.isEmpty
                        {
                            ...
                            ContentUnavailableView(
                                ...
                                description: Text(
                                    "Tidak ada catatan kas untuk bulan ini."
                                )
                            )
                        } else {
                            LazyVStack(spacing: 0) {
                                ForEach(viewModel.filteredRecords, id: \.id) {
                                    record in
                                    // ✅ INTERAKSI KLIK TRANSAKSI
                                    Button(action: {
                ...
                ToolbarItem(placement: .primaryAction) {
                    Button {
                        if let url = PDFGeneratorService.generateCashflowReport(
                            ...
                            filename:
                                "Laporan_Kas_\(viewModel.currentMonthString.replacingOccurrences(of: " ", with: "_"))"
                        ) {
            ...
            .onAppear {
                Task { await viewModel.loadRecords(branchId: branchId) }
            }
            ...
            .alert(
                ...
                if let record = selectedRecord {
                    ...
                    let dateStr = record.timestamp.formatted(
                        date: .abbreviated,
                        time: .shortened
                    )

                    Text(
                        "\(status) sejumlah \(record.amount.toRupiah())\nTanggal: \(dateStr)\n\nCatatan:\n\(noteStr)"
                    )
                }
            }
        }
