        ...
        ScrollView {
            VStack(spacing: 24) {
                if viewModel.isLoading {
                    ProgressView("Mengolah algoritma data...").frame(maxWidth: .infinity, minHeight: 300)
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack {
                            if viewModel.chartData.isEmpty {
                                Text("Belum ada data penjualan pada kategori ini.").foregroundColor(.gray)
                            } else {
                    ...
                    AnalyticSection(title: "Paling Laris (Best Seller)", icon: "flame.fill", iconColor: Color(hex: "#F59E0B")) {
                        ForEach(Array(viewModel.bestSellers.prefix(3).enumerated()), id: \.element.productName) { index, item in
                            AnalyticRow(rank: index + 1, name: item.productName, detail: "\(item.quantitySold) Terjual", highlightColor: Color(hex: "#00A3A3"))
                        }
                    }
                    ...
                    AnalyticSection(title: "Perlu Evaluasi", icon: "arrow.down.right.circle.fill", iconColor: Color(hex: "#EF4444")) {
                        ForEach(Array(viewModel.leastPopular.prefix(3).enumerated()), id: \.element.productName) { index, item in
                            AnalyticRow(rank: index + 1, name: item.productName, detail: "Hanya \(item.quantitySold) Terjual", highlightColor: Color(hex: "#EF4444"))
                        }
                    }
                }
            }
